...
public class Detalhes {
    ...
    //Um stream é uma sequência de elementos que pode ser processada em paralelo ou em série
    //Pode ser criada a partir de uma coleção, um array, um arquivo, entre outros.

    //As operações intermediárias são aquelas que pode ser aplicadas em uma stream e retornam uma nova stream como resultado
    //Essas operações não são executadas imediatamente, mas apenas quando uma operação final é chamada
    ...
    //COLLECT: permite coletar os elementos da stream em uma coleção ou em outro tipo de dados.
   ...
   // Além das operações intermediárias e finais existem muitas outras disponíveis, como:
    ...
    public void testaStream(){
        List<String> nomes2 = Arrays.asList("Jacque", "Iasmin", "Paulo", "Rodrigo", "Nico");

        //ordenar
        nomes2.stream() //stream = fluxo de dados -> novo fluxo de dados
